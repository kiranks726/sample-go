...
deploy: update-config ## Deploy stack as complete build to AWS environment
...
data-import:	## Import test data into environment
	...
	cd backend/mainmodule; go run ${DATA_IMPORT_CMD_PATH} -u=${RESOURCE_URL} -p=${CSV_FILE}
data-import-fast:	## Fast Import test data into environment
	cd backend/mainmodule; go run ${DATA_IMPORT_CMD_PATH} -f -u=${RESOURCE_URL} -p=${CSV_FILE}
